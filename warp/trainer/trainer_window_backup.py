...
class WarpCoreWindow(QMainWindow):
    ...
    def _update_screen_type_badge(self, stype: str):
        """Aktualizuje wizualny wskaźnik typu ekranu."""
        # W Twoim UI ten element nazywa się prawdopodobnie self._type_lbl lub podobnie
        # Jeśli nie masz dedykowanego widgetu, logujemy to tylko do statusu:
        icon = SCREEN_TYPE_ICONS.get(stype, '❓')
        label = SCREEN_TYPE_LABELS.get(stype, 'Unknown')
        self.statusBar().showMessage(f"Current Screen: {icon} {label}", 5000)

    def _save_screen_type_example(self, path: Path, stype: str):
        """Główna funkcja zapisu wywoływana przez menu kontekstowe."""
        try:
            # 1. Zapis do bazy danych
            self._data_mgr.set_screen_type(path, stype)

            # 2. Aktualizacja lokalnych słowników w oknie
            self._screen_types[path.name] = stype
            self._screen_types_manual.add(path.name)

            # 3. Odświeżenie listy plików (aby ikony przy nazwach się zmieniły)
            self._update_progress()

            # 4. Odświeżenie UI jeśli to obecnie wybrany plik
            if self._current_idx >= 0 and self._screenshots[self._current_idx] == path:
