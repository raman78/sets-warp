...
class TrainingDataManager:
    ...
    def __init__(self, data_dir: Path):
        ...
        self._screen_metadata: dict[str, dict] = {}
    ...
    def set_screen_type(self, path: Path, stype: str):
        """Zapisuje ręcznie ustawiony typ ekranu dla danego pliku."""
        key = path.name

        # Inicjalizujemy metadane jeśli ich nie ma
        if not hasattr(self, '_screen_metadata'):
            self._screen_metadata = {}

        if key not in self._screen_metadata:
            self._screen_metadata[key] = {}

        self._screen_metadata[key]['screen_type'] = stype
        self._dirty = True

        # Zapisujemy do pliku, aby dane nie zniknęły
        self.save()
        logger.info(f"Saved screen type {stype} for {key}")

    def get_screen_type(self, path: Path) -> str:
        """Pobiera zapisany typ ekranu."""
        return self._screen_metadata.get(path.name, {}).get('screen_type', 'UNKNOWN')
