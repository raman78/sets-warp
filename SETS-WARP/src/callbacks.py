...
def _restore_session_slots(self):
    ...
    for key, saved_val in saved['equipment'].items():
        ...
        if isinstance(current, list) and isinstance(saved_val, list):
            ...
            for i, item in saved_val.items():
                if (current.get(i) == '' or current.get(i) is None) and item not in ('', None):
    ...
    for boff_id, saved_boff in enumerate(saved['boffs']):
        if boff_id >= len(s['boffs']):
            break
        if not saved_boff['spec']:
            continue
        saved_prof = saved_boff['spec'][0] if len(saved_boff['spec']) > 0 else ''
        saved_spec = saved_boff['spec'][1] if len(saved_boff['spec']) > 1 else ''
        ...
        for ability_idx, saved_ability in enumerate(saved_boff['abilities']):
            if ability_idx >= len(s['boffs'][boff_id]):
                break
            current_ability = s['boffs'][boff_id][ability_idx]
            if (current_ability == '' or current_ability is None) and saved_ability not in ('', None):
