...
DESKTOP_FILE="$DESKTOP_DIR/sets-warp.desktop"

# ── Uninstall ─────────────────────────────────────────────────────────────────
if [ "${1}" = "--uninstall" ]; then
    echo "Uninstalling SETS-WARP desktop entry..."
    rm -f "$DESKTOP_FILE"
...
Comment=Star Trek Online build planner with WARP screenshot recognition
Exec=$EXEC
Icon=$ICON_NAME
Terminal=false
Type=Application
...
echo "To uninstall: bash installer/install_desktop.sh --uninstall"
