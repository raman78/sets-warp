...
def _elog(msg):
    ...
    _early_log.write(line)
    _early_log.flush()
    os.fsync(_early_log.fileno())
    print('[SETS] ' + msg, flush=True)

_elog('main: start, log=' + _early_log_path)
_elog('main: importing stdlib')
...
try:
    ...
    _elog(f'main: IMPORT ERROR — {_import_err}')
    _missing = str(_import_err)

    # IMPORTANT: At this point the process may have partially initialised XCB/Qt,
    # which makes it impossible to safely open any GUI window here.
    # Solution: spawn a completely fresh process to show the repair dialog,
    # wait for the user's choice, then act on the exit code.
    #
    # Exit codes from the repair helper:
    #   0 = repair completed (or user confirmed) → re-exec bootstrap
    #   2 = user clicked Exit
    #   other = helper itself crashed → fall back to sys.exit

    import subprocess as _sp

    _bootstrap = os.path.join(_script_dir, 'bootstrap.py')

    # Find a system Python (not the venv one — it may be broken) to run the helper
    _system_py = None
    for _cand in ('python3.13', 'python3.12', 'python3.11', 'python3', 'python'):
        try:
            _r = _sp.run([_cand, '--version'], capture_output=True, timeout=5)
            if _r.returncode == 0:
                _system_py = _cand
                break
        except Exception:
    ...
    if _system_py:
        # Spawn fresh process: bootstrap --repair shows tkinter dialog, runs pip, exits
        _r = _sp.run([_system_py, _bootstrap, '--repair'], timeout=600)
        if _r.returncode == 2:
            sys.exit(2)   # user chose Exit
        elif _r.returncode == 0:
            # Repair done — re-exec via bootstrap with venv Python so it relaunches cleanly
            os.execv(sys.executable, [sys.executable, _bootstrap] + sys.argv[1:])
        else:
            _elog(f'main: repair process exited with code {_r.returncode}')
            sys.exit(1)
    else:
        _elog('main: no system Python found for repair — exiting')
        sys.exit(1)
