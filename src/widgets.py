...
from PySide6.QtCore import QEvent, QMetaObject, QObject, QPoint, QRect, QSize, Qt, QThread, Signal, Slot
from PySide6.QtGui import QBrush, QColor, QCursor, QEnterEvent, QImage, QMouseEvent, QPainter, QPen, QPixmap
...
class WidgetStorage():
    ...
    def __init__(self):
        ...
        self.progress_bar = None       # QProgressBar — created in setup_splash
        self.progress_detail = None    # QLabel — created in setup_splash
...
class ImageLabel(QLabel):
    """
    Label displaying image that resizes according to its parent's width while preserving aspect
    ratio. Uses QLabel.setPixmap — no custom paintEvent, no QPainter.
    """
    def __init__(self, path: str = '', aspect_ratio: tuple[int, int] = (0, 0), *args, **kwargs):
        ...
        if path == '':
            ...
            self.p = QImage(path)
            if self._w == 0 and not self.p.isNull():
                self._w = self.p.width()
                self._h = self.p.height()
    ...
    def _update_pixmap(self):
        if self.p.isNull() or self._w == 0:
            return
        w = self.width() if self.width() > 0 else self._w
        h = int(w * self._h / self._w)
        scaled = QPixmap.fromImage(self.p).scaled(
            w, h,
...
class ShipImage(QLabel):
    """
    Label displaying a ship image, scaled to fit a fixed 500x280 frame
    while preserving aspect ratio. Centered within the frame.
    """
    FRAME_W = 500
    FRAME_H = 280
