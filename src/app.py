import sys
print('[SETS] src/app.py: start', flush=True)

print("[SETS] app.py: importing os/pathlib", flush=True)
import os
from pathlib import Path
print("[SETS] app.py: importing PySide6", flush=True)
...
print("[SETS] app.py: importing local modules", flush=True)
...
log.info("app.py: module-level imports done")
...
class SETS():
    ...
    def __init__(self, theme, args, path, config, versions):
        ...
        _env = read_env_file(
            ...
            ['SETS_CF_CLEARANCE', 'SETS_USER_AGENT'])
        _headers = {'User-Agent': (
            _env.get('SETS_USER_AGENT') or
            'Mozilla/5.0 (X11; Linux x86_64; rv:124.0) Gecko/20100101 Firefox/124.0')}
        _cookies = {}
        if 'SETS_CF_CLEARANCE' in _env:
        ...
        log.info('SETS.__init__: ImageManager OK')
        self.app, self.window = self.create_main_window()
        log.info('SETS.__init__: create_main_window OK')
        self.cache_icons()
        log.info('SETS.__init__: cache_icons OK')
        self.cache_item_aliases()
        log.info('SETS.__init__: cache_item_aliases OK')
        self.building = True
        self.build = self.empty_build()
        log.info('SETS.__init__: empty_build OK')
        self.export_window = ExportWindow(self, self.window, self.get_build_markdown)
        log.info('SETS.__init__: ExportWindow OK')
        self.setup_main_layout()
        log.info('SETS.__init__: setup_main_layout OK')
        ...
        log.info('SETS.__init__: Picker OK')
        self.edit_window = ItemEditor(self, self.window)
        log.info('SETS.__init__: ItemEditor OK')
        self.ship_selector_window = ShipSelector(self, self.window)
        log.info('SETS.__init__: ShipSelector OK')
    ...
    def cache_icons(self):
        ...
        p.drawText(na.rect(), Qt.AlignmentFlag.AlignCenter, 'N/A')
        p.end()
        self.cache.na_image = na
    ...
    def setup_splash(self, frame: QFrame):
        ...
        from PySide6.QtWidgets import (
            QProgressBar, QFrame as _QF,
            QVBoxLayout as _VBL, QHBoxLayout as _HBL)
        from PySide6.QtGui import QPixmap as _QPixmap

        root = _VBL()
        root.setContentsMargins(0, 100, 0, 0)
        root.setSpacing(12)
        root.setAlignment(ATOP)

        def _center_row(widget):
            row = _HBL()
        ...
        _img_path = get_asset_path('sets_loading.png', self.app_dir)
        _pix = _QPixmap(_img_path)
        if not _pix.isNull():
            _pix = _pix.scaled(
                BANNER_W - 16, BANNER_H - 16,
                Qt.AspectRatioMode.KeepAspectRatio,
                Qt.TransformationMode.SmoothTransformation)
        logo_label.setPixmap(_pix)
        banner_inner.addWidget(logo_label)
        root.addLayout(_center_row(banner_frame))

        # ── PROGRESS FRAME  1000 x 500 ────────────────────────────────────
        ...
        prog_inner.setAlignment(ATOP)

        loading_label = self.create_label('Loading: ...', 'label_subhead')
        loading_label.setAlignment(AHCENTER)
        self.widgets.loading_label = loading_label
        prog_inner.addWidget(loading_label)
        ...
        progress_bar.setStyleSheet(
            f'QProgressBar {{'
            f'  background-color: {self.theme["defaults"]["lbg"]};'
            f'  border: none;'
            f'  border-radius: 9px;'
            f'  color: {self.theme["defaults"]["fg"]};'
            f'  font-size: 10px;'
            f'  text-align: center;'
            f'}}'
            f'QProgressBar::chunk {{'
            f'  background-color: {self.theme["defaults"]["sets"]};'
            f'  border-radius: 9px;'
            f'}}'
        )
        progress_bar.hide()
        self.widgets.progress_bar = progress_bar
        prog_inner.addWidget(progress_bar)

        progress_detail = self.create_label('', 'hint_label')
        progress_detail.setAlignment(AHCENTER)
